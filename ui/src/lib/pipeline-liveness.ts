...
export interface LivenessBannerLink {
  /** Issue link target. We only link issues; cases lack a routable id here. */
  issueId: string;
...
export interface LivenessBannerView {
  ...
  title: string;
  body: string;
  /** Primary link to the underlying blocker issue, when one is known. */
  blockerLink: LivenessBannerLink | null;
  /** Secondary link to the linked automation/work issue, when one is known. */
